...
import oneocr.cli.DualProgressRenderer;
import oneocr.cli.OcrTool;
import oneocr.cli.ProgressTracker;
import oneocr.cli.ProgressAwareLogFormatter;
import oneocr.cli.folder.FileProcessor.FileType;
import oneocr.cli.layout.LayoutCliOptions;
import oneocr.cli.layout.LayoutExport;
import oneocr.cli.layout.PageLayout;
...
public class FolderOcrCommand implements Callable<Integer> {
    ...
    private int getActualPageCount(WorkItem workItem) {
        ...
        if (workItem.fileType() == FileProcessor.FileType.PDF) {
            try {
                var pdfInfo = oneocr.cli.pdf.PdfInfoUtil.getPdfInfo(workItem.filePath().toFile());
